...
# Fire and Rescue Service response times here is defined as the time station received the phone call to the time on scene
# Collecting data from three source: North, Mid-and west, and South


# Part1: North
# Loading data
# 2019, 422 rows
north_2019 <- read_excel("data/on-disk/Fire-and-Rescue-Service-response-times/North Response times by Lower Layer Super Output Area.xlsx", 
    sheet = "2019", skip = 1)
# 2020, 421 rows
north_2020 <- read_excel("data/on-disk/Fire-and-Rescue-Service-response-times/North Response times by Lower Layer Super Output Area.xlsx", 
    sheet = "2020", skip = 1)
# 2021, 422 rows
north_2021 <- read_excel("data/on-disk/Fire-and-Rescue-Service-response-times/North Response times by Lower Layer Super Output Area.xlsx", 
    sheet = "2021", skip = 1)
    
# Full join threee years, 426 rows.
# **Some rescure time are missing**, but I choose to calculate the avg rescure time by ignore NA
north_three_years <- 
    north_2019 %>% rename(avg_datatime_2019 = "Average of Difference between Call Time Stamp and Arrive Time Stamp") %>%
    full_join(
